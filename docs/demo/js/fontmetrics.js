...
(function() {
	...
	var SAMPLE_RATE = config.sampleRate || 44100;
        ...
        var onError = config.onError || function(e){console.log(e);};
        var onAudioProcess = config.onAudioProcess || function(sampleRate, numChannels, floatSamples){};
        var onRecord = config.onRecord || function(numChannels, sampleRate){};
        ...
        function resume() {
            try {
                ...
                if (audioCtx.createJavaScriptNode) {
                    audioNode = audioCtx.createJavaScriptNode(bufferSize, audioChannels, audioChannels);
                } else if (audioCtx.createScriptProcessor) {
                    audioNode = audioCtx.createScriptProcessor(bufferSize, audioChannels, audioChannels);
                } else {
                ...
                audioNode.onaudioprocess = function(e) {
                    ...
                    try {
                        //console.log("before interleave ", e);
                        var floatSamples = interleave(e);
                        //console.log(audioCtx.sampleRate, floatSamples);
                        if (isEdge || isIE) {
                        ...
                        onAudioProcess(Math.floor(audioCtx.sampleRate), audioChannels, floatSamples);
                    } catch (e) {
                        onError(e+"");
                    }
        ...
        function record(notAllowedCallback) {
	        console.log("in record() ios13? " + isIOS13());
            if (recording) {
            ...
            navigator.mediaDevices.getUserMedia({
                ...
                try {
					...
					console.log("Calling onRecord after successful getUserMedia");
                    onRecord(audioChannels, Math.floor(audioCtx.sampleRate));
                    audioNode.onaudioprocess = function(e) {
                        if (!recording) {
                            return;
                ...
                if (err.name == 'NotAllowedError' && notAllowedCallback) {
                    notAllowedCallback();
                    return;
                }
